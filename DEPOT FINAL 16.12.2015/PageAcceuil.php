...
if (isset ($_POST['valider'])){
		...
		//si les valeurs sont identiques (qu'on a un résultat)
			if ($res->num_rows==1){
				...
				//TABLE CHEF EQUIPE ?
					$sql = "SELECT * FROM chefequipe WHERE prenom ='$prenom' && nom='$nom' ";
				...
				//si les valeurs sont identiques (qu'on a un résultat)
					if ($res->num_rows==1){
						...
						header('Location:PageChefEquipe.php');
									...
									$sql = "SELECT * FROM equipier WHERE prenom ='$prenom'&& nom='$nom'";
								//On lance la commande 
									$res = $conn->query($sql);
								
								//si les valeurs sont identiques (qu'on a un résultat)
									if ($res->num_rows==1){
										//récupérons le nom de l'équipe
										$row=$res->fetch_assoc();
										$nomEquipe = $row['nomEquipe'];
										$_SESSION['nomEquipe']=$nomEquipe;
